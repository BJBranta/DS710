...
def find_files_student_name_and_ending(name, file_ending, allow_previous=False):
    return [
        file
        for file in find_files_with_ending(file_ending, allow_previous)
...
possible_generated_files = ["assign7_task1-4.png", "assign7_task2-5.png"]
...
def test_verify_no_hardcoded_paths(submitted_source_code_as_lines):
    ...
    for line in submitted_source_code_as_lines:
        ...
        for A in string.ascii_uppercase:
            assert A + ":/" not in line_before_hash, (
...
def test_verify_no_dot_show(submitted_source_code_as_lines):
    ...
    for line in submitted_source_code_as_lines:
        ...
        assert ".show()" not in line_before_hash, (
            "Do not leave `.show()` in your submitted code; we will reject it.  rely on file saving instead of .show()"
        )
...
class TestTask1MappingLakes:
    ...
    def test_have_plot(self, student_code):
        assert (
            len(
                find_files_student_name_and_ending(
                    student_code.last_name, "assign7_task1-4.png", allow_previous=False
                )
            )
            > 0
        ), (
            "i didn't find a file that ends with 'assign7_task1-4.png' and starts with your last name."
        )
...
class TestTask2GraphingTowers:
    ...
    def test_height_floors_type(self, student_code):
        buildings_df = student_code.buildings_df
        assert isinstance(buildings_df["Height"].iloc[0], int) | isinstance(
            buildings_df["Height"].iloc[0], np.int64
        )
    ...
    def test_have_plot(self, student_code):
        assert (
            len(
                find_files_student_name_and_ending(
                    student_code.last_name, "assign7_task2-5.png", allow_previous=False
                )
            )
            > 0
        ), (
            "i didn't find a file that ends with 'assign7_task2-5.png' and starts with your last name."
        )
