...
possible_generated_files = ['exception_should_have_been_raised',
                            'am_table_500_100_5_csv',
                            'am_table_500_100_5_tsv',
                            'am_table_500_100_5_aligned']

def remove_files_from_previous_run():
    import itertools
    for extension in itertools.product(['','.txt','.csv','.tsv'],repeat=4):
        for filename in possible_generated_files:
            remove_file_if_exists(filename+''.join(extension))

remove_files_from_previous_run()
...
class TestTask4AmortizationNoFile:
    ...
    def test_given_case_500_1_005_insufficient_payment_raises(self, student_code):
        """
        make sure an exception is raised in the infinite loop case.
        """
...
class TestTask4AmortizationFormattingFunctions:
    ...
    def test_format_csv_numbers(self, student_code):
        format_csv = student_code.format_csv
        assert format_csv(3,100.123123,12.1778212,20192.1029873) == "3,100.12,12.18,20192.10\n"
    ...
    def test_format_tsv_numbers(self, student_code):
        format_tsv = student_code.format_tsv
        assert format_tsv(3,100.123123,12.1778212,20192.1029873) == "3\t100.12\t12.18\t20192.10\n"
...
class TestTask4AmortizationYesFileCSV:
    ...
    def test_given_case_500_100_005_produces_txt_file(self, student_code):
        amortization = student_code.amortization
        remove_files_from_previous_run()

        amortization(500, 100, 0.05, 'am_table_500_100_5_csv')
        assert Path('am_table_500_100_5_csv.txt').exists() and "your code should have generated a .txt file when called with a filename"
