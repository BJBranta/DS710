...
possible_generated_files = [
    ...
    "assign8_task2-5.png",
    "assign8_task2-6.png",
    "assign8_task2-6-1.png",
    "assign8_task2-6-2.png",
    "assign8_task2-6-3.png"
    ]
...
@pytest.mark.skipif(not student_code_exists(), reason=f"specified code file {with_dotpy(student_code_filename)} doesn't exist")
class TestTask1Scrabble:

    def test_has_english_scores(self, student_code):
        assert isinstance(student_code.english_letter_scores,dict)

    def test_has_german_scores(self, student_code):
        assert isinstance(student_code.german_letter_scores,dict)


    def test_has_word_score(self, student_code):
        assert callable(student_code.word_score)

    def test_word_score_english_THE(self,student_code):
        word_score = student_code.word_score
        assert word_score("THE",student_code.english_letter_scores) == 6

    def test_word_score_german_Ziege(self,student_code):
        word_score = student_code.word_score
        assert word_score("Ziege",student_code.german_letter_scores) == 8

    def test_has_word_counts_from_file(self,student_code):
        assert callable(student_code.word_counts_from_file)
    
    def test_word_counts_from_file_makes_df(self,student_code):
        word_counts_from_file = student_code.word_counts_from_file
        assert isinstance(word_counts_from_file("example_text.txt"),pd.DataFrame)

    def test_has_add_score_column(self,student_code):
        assert callable(student_code.add_score_column)

    def test_add_score_column_returns_None(self,student_code):
        add_score_column = student_code.add_score_column
        example_df = student_code.word_counts_from_file("example_text.txt")
        assert add_score_column(example_df, student_code.english_letter_scores,"GRADER TEST COLUMN") is None

    def test_add_score_column_adds_column(self,student_code):
        add_score_column = student_code.add_score_column
        example_df = student_code.word_counts_from_file("example_text.txt")
        add_score_column(example_df, student_code.english_letter_scores,"GRADER TEST COLUMN")
        assert "GRADER TEST COLUMN" in example_df.columns

    def test_has_score_data(self,student_code):
        assert callable(student_code.score_data)

    def test_score_data_my_df(self,student_code):
        score_data = student_code.score_data
        my_df = pd.DataFrame({
            "Word":["a","b","c"],
            "Count":[1,1,5],
            "Score":[6,5,2]
            })
        assert score_data(my_df,"Count","Score") == (21,3,2)

    def test_score_data_my_df_remove_word(self,student_code):
        score_data = student_code.score_data
        my_df = pd.DataFrame({
            "Word":["a","b","c"],
            "Count":[1,1,5],
        ...
        assert score_data(my_df,"Count","Score",["c"]) == (18,9,9)
...
@pytest.mark.skipif(not student_code_exists(), reason=f"specified code file {with_dotpy(student_code_filename)} doesn't exist")
class TestTask2Sentiment:
    
    def test_has_wikipedia_page_content(self, student_code):
        assert callable(student_code.wikipedia_page_content)

            
    def test_wikipedia_page_content_returns_string(self, student_code):
        wikipedia_page_content = student_code.wikipedia_page_content
        assert isinstance(wikipedia_page_content("Aretha Teckentrup"),str)

                
    def test_wikipedia_page_content_values(self, student_code):
        wikipedia_page_content = student_code.wikipedia_page_content
        assert wikipedia_page_content("Haesun Park")[:12] == "Haesun Park "

    def test_has_interstate_df(self,student_code):
        assert isinstance(student_code.interstate_df,pd.DataFrame)

    def test_instate_df_has_title_length_formed_colums(self,student_code):
        interstate_df = student_code.interstate_df
        has_Title = "Title" in interstate_df.columns
        has_Length_mi = "Length(mi)" in interstate_df.columns
        has_Formed = "Formed" in interstate_df.columns
        assert has_Title and has_Length_mi and has_Formed


    def test_instate_df_has_page_contents_column(self,student_code):
        interstate_df = student_code.interstate_df
        assert "Page Contents" in interstate_df.columns
        
    def test_interstate_df_contents(self, student_code):
        interstate_df = student_code.interstate_df
        assert 'Interstate 11' in list(interstate_df.Title)
        assert interstate_df.loc[interstate_df[interstate_df.Title == 'Interstate 11'].index[0]]['Page Contents'][:50] == 'Interstate 11 (I-11) is an Interstate Highway that'
        assert abs(interstate_df.loc[interstate_df[interstate_df.Title == 'Interstate 11'].index[0]].neg - 0.013) < 0.001
        assert interstate_df.loc[interstate_df[interstate_df.Title == 'Interstate 11'].index[0]]['Formed'] == 2017

    

    def test_instate_df_has_sentiment_colums(self,student_code):
        interstate_df = student_code.interstate_df
        has_neg = "neg" in interstate_df.columns
        has_pos = "pos" in interstate_df.columns
        has_neu = "neu" in interstate_df.columns
        has_compound = "compound" in interstate_df.columns
        assert has_neg and has_neu and has_pos and has_compound

    def test_have_plot_2_4(self, student_code):
        assert len(find_files_student_name_and_ending(student_code.last_name,'assign8_task2-4.png',allow_previous=False))>0 and "i didn't find a file that ends with 'assign8_task2-4.png' and starts with your last name."

    def test_have_plot_2_5(self, student_code):
        assert len(find_files_student_name_and_ending(student_code.last_name,'assign8_task2-5.png',allow_previous=False))>0, "i didn't find a file that ends with 'assign8_task2-5.png' and starts with your last name."

    def test_have_plot_2_6(self, student_code):
        assert len(find_files_student_name_and_ending(student_code.last_name,'assign8_task2-6.png',allow_previous=False))>0 or len(find_files_student_name_and_ending(student_code.last_name,'assign8_task2-6-1.png',allow_previous=False))>0, "i didn't find a file that ends with 'assign8_task2-6.png' and starts with your last name."
